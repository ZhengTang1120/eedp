...
for i in range(19,29):
	print (i)
	start = time.time()
	os.system(f'python parse.py outm21/parser{i+1:03} brat_dev.conllx bratdevoutm.conllx')
	end = time.time()
	print(f'time: {end-start:,.2f} secs')
	os.system(f'python conllx2brat.py bionlp09_shared_task_development_data_rev1m/ --conllx bratdevoutm.conllx')
	end = time.time()
	print(f'time: {end-start:,.2f} secs')
	os.system("./a2-evaluate.pl -g gold-dev/ -s -p bionlp09_shared_task_development_data_rev1m/*.t1")
	end = time.time()
	print(f'time: {end-start:,.2f} secs')
